...
export default function PrivacyPolicyContent({ topic }) {
  const locale = useLocale();
  const t = useTranslations();

  const title = localizedField(topic.title, locale);
  const content = localizedField(topic.privacy_policy, locale);
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', position: 'relative' }}>
      ...
      <article className="container" style={{ maxWidth: 780, paddingTop: 48, paddingBottom: 80, position: 'relative', zIndex: 1 }}>
        ...
        <p style={{ fontFamily: 'var(--sans)', fontSize: 15, color: 'var(--text-3)', marginBottom: 40 }}>
          {title}
        </p>
